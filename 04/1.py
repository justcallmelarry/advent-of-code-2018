...
class AdventOfCode:
    ...
    def run(self):
        ...
        for guard in self.schedule:
            ...
            guard_sum = matrix.sum()
            if guard_sum > sleepy_guard_sum:
                sleepy_guard = guard
                sleepy_guard_sum = guard_sum

        matrix = numpy.matrix(self.schedule.get(sleepy_guard).get('wake_matrix'))
        sums = matrix.sum(axis=0)
        sums = sums.tolist()
        minute_value = max(sums[0])
        minute = sums[0].index(minute_value)
        self.output(f'guard: {sleepy_guard}', f'minute: {minute}', f'answer: {int(sleepy_guard) * int(minute)}')

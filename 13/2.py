...
class AdventOfCode:
    ...
    def load_input(self):
        if any([x in sys.argv for x in ('--test', '-t')]):
            self.input = os.path.join(self.filepath, 'sample2.txt')
            self.test = True
    ...
    def run(self):
        ...
        while len(self.carts) > 1:
            ...
            self.carts = [x for x in self.carts if (x.x, x.y) not in crashes]
            ticks += 1
        self.output(ticks)
        winner = self.carts[0]
        self.output(f'{winner.x},{winner.y}')
...
class Cart:
    ...
    def __repr__(self):
        return f'Cart {self._id} <{self.x}, {self.y}>'

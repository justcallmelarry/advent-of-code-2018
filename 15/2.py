...
def main():
    ...
    while winners != 'elf':
        ...
        while len(elves) > 0 and len(goblins) > 0:
            if len(elves) != elf_len:
                break
            temp_units = sorted(units, key=lambda x: (x.y, x.x))
            sorted_elves = sorted(elves, key=lambda x: (x.y, x.x))
        ...
        if len(elves) != elf_len:
            winners = None
    print(winners, ap, rounds, total_hp, total_hp * rounds)
